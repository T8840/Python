...
result, cj = request_ajax_url(f"http://{ip}/login/Auth",
...
while True:
    ...
    print("data:" + data)
    result = request_ajax_url(f"http://{ip}/goform/GetRouterStatus", data.encode('utf-8'), None,
                              "password=%s" % cookiePassword)
    print(result[0].read())
    result = request_ajax_url(f"http://{ip}/goform/WifiApScan", data.encode('utf-8'), None,
                              "password=%s" % cookiePassword)
    print(result[0].read())
    time.sleep(1)